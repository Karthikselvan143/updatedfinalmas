...
export class DatepickerComponent implements OnInit {
  ...
  private apiUrl = 'http://localhost:5046/api/DatePicker';
  ...
  getDates(): void {
    this.http.get(`${this.apiUrl}/GetDate`).subscribe(
      (data: any) => {
        this.dates = data;
      },
      (error: any) => {
  ...
  applyDateRange(): void {
    if (this.newDatePicker.startdate && this.newDatePicker.enddate && this.newDatePicker.location) {
      ...
      this.http.post(`${this.apiUrl}/CreateDate`, this.newDatePicker, httpOptions).subscribe(
        (response: any) => {
